<template>
  <section class="table-panel task-page" :class="inspectionType">
    ...
    <div class="toolbar"><div class="toolbar-left"><el-input v-model="keyword" clearable placeholder="任务编号、桥梁编号或状态" style="width:280px" @keyup.enter="load"/><el-button @click="load">查询</el-button></div><div class="toolbar-right"><el-button v-if="canPlan&&inspectionType==='periodic'" @click="generatePeriodicTasks">检查并自动生成</el-button><el-button v-if="canPlan&&inspectionType==='initial'" type="primary" @click="openCreate">创建初始检查任务</el-button></div></div>
    ...
    <el-table :data="rows" border height="560">
      ...
      <el-table-column label="操作" fixed="right" :width="canPlan||canReview?300:180">
        <template #default="{row}">
          ...
          <el-popconfirm v-if="canPlan&&canDelete(row)" title="确认删除该任务？关联的分配记录和状态历史将一并删除。" confirm-button-text="删除" cancel-button-text="取消" @confirm="remove(row)"><template #reference><el-button text type="danger">删除</el-button></template></el-popconfirm>
    ...
    <el-drawer v-model="drawer" :title="editing?'编辑任务':'创建任务'" size="620px">
      <el-form label-position="top" class="form-grid">
        ...
        <el-form-item label="桥梁" class="full-row"><el-select v-model="form.bridge_code" filterable remote reserve-keyword clearable :remote-method="searchBridges" :loading="bridgeLoading" placeholder="输入桥梁编号、名称、路线或地址搜索" style="width:100%"><el-option v-for="item in bridgeOptions" :key="item.bridge_code" :label="bridgeLabel(item)" :value="item.bridge_code"><div class="bridge-option"><b>{{ item.bridge_code }} · {{ item.bridge_name }}</b><span>{{ item.route_name || item.route_code || '未配置路线' }} · {{ item.bridge_type_name || '未配置桥型' }} · {{ item.location_address || '未填写地址' }}</span></div></el-option></el-select><p class="field-tip">选择后将保存桥梁编号；无需手工记忆或输入编号。</p></el-form-item>
        <el-form-item label="检查类型"><el-input :model-value="inspectionType==='initial'?'初始检查':'定期检查（由系统自动生成）'" disabled/></el-form-item>
        ...
        <el-form-item label="状态"><el-input :model-value="form.task_status||'待分配'" disabled/></el-form-item>
...
const rows=ref([]),total=ref(0),page=ref(1),size=ref(10),keyword=ref(''),drawer=ref(false),saving=ref(false),editing=ref(null),form=reactive({}),bridgeOptions=ref([]),bridgeLoading=ref(false)
...
async function openCreate(){editing.value=null;Object.assign(form,{task_id:'',bridge_code:'',inspection_type:'initial',inspection_level:'Ⅰ',task_status:'待分配',plan_start_date:'',plan_end_date:'',remarks:''});await searchBridges('');drawer.value=true}
...
async function searchBridges(query){bridgeLoading.value=true;try{const data=await http.get('/bridges',{params:{page:1,size:60,bridgeSearch:query||''}});bridgeOptions.value=data.records}finally{bridgeLoading.value=false}}
function bridgeLabel(item){return `${item.bridge_code} · ${item.bridge_name} · ${item.route_name||item.route_code||'未配置路线'}`}
...
async function remove(row){await http.delete(`/tasks/${row.task_id}`);ElMessage.success('任务已删除');await load()}
async function generatePeriodicTasks(){const result=await http.post('/tasks/periodic/generate',{});ElMessage.success(result.message||`已自动生成 ${result.created||0} 条任务`);await load()}
...
function canDelete(row){return ['待分配','已取消'].includes(row.task_status)}
function statusDot(status){if(status==='已审核'||status==='已完成')return'ok-dot';if(status==='已取消')return'danger-dot';return'warn-dot'}
onMounted(load)
</script>
<style scoped>
.task-heading{display:flex;justify-content:space-between;align-items:center;padding:4px 0 16px;margin-bottom:14px;border-bottom:1px solid #dbe3ec}.task-heading span{font-size:11px;font-weight:700;color:#0f766e}.task-heading h2{margin:5px 0;font-size:23px}.task-heading p{margin:0;color:#64748b}.task-page.periodic .task-heading span{color:#c2410c}.task-page.periodic :deep(.el-table th){--el-table-header-bg-color:#fff7ed}.task-page.initial :deep(.el-table th){--el-table-header-bg-color:#eff6ff}.toolbar-right{display:flex;gap:8px}.pager{display:flex;justify-content:flex-end;margin-top:14px}.locked-text{font-size:12px;color:#94a3b8}.form-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:5px 16px}.full-row{grid-column:1/-1}.bridge-option{display:flex;flex-direction:column;gap:3px;line-height:1.35;padding:4px 0}.bridge-option span,.field-tip{font-size:12px;color:#64748b}.field-tip{margin:7px 0 0;line-height:1.5}@media(max-width:680px){.task-heading{align-items:flex-start;flex-direction:column;gap:10px}.form-grid{grid-template-columns:1fr}}
</style>
