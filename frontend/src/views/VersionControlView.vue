...
async function rollback(row){await ElMessageBox.confirm(`系统将回溯到 ${row.version_no}。执行前会创建安全恢复分支并备份数据库，完成后需要重启系统。`,'确认版本回溯',{type:'warning',confirmButtonText:`回溯到 ${row.version_no}`,cancelButtonText:'取消'});rollingBack.value=row.version_no;try{const result=await http.post(`/version-control/versions/${encodeURIComponent(row.version_no)}/rollback`);ElMessage.success(result.message);await load()}finally{rollingBack.value=''}}
